...
namespace Kitty
{
    public partial class KittyMain : CombatRoutine
    {
        ...
        public static async Task<bool> BearRotationCoroutine()
        {
            ...
            if (await RemoveRooted(BEAR_FORM, MeIsRooted && gotTarget && !IsInMeleeRange(Me.CurrentTarget))) return true;
            ...
            if (await Cast(SKULL_BASH, gotTarget && SkullBashConditions(Me.CurrentTarget) && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(INCAPACITATING_ROAR, gotTarget && IncapacitatingRoarConditions(Me.CurrentTarget) && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(TYPHOON, gotTarget && TyphoonConditions(Me.CurrentTarget) && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(MIGHTY_BASH, gotTarget && MightyBashConditions(Me.CurrentTarget) && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(WAR_STOMP, gotTarget && WarStompConditions(Me.CurrentTarget) && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await CastBuff(HEALING_TOUCH, Me.HealthPercent <= 90 && IsOverlayed(DREAM_OF_CENARIUS_INT))) return true;
            ...
            if (await CastBuff(BERSERK, gotTarget && BerserkBearConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await CastBuff(INCARNATION_BEAR, gotTarget && IncarnationBearConditions && IsInMeleeRange(Me.CurrentTarget)))
            if (await Cast(FORCE_OF_NATURE, gotTarget && ForceOfNatureConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            ...
            if (await CastGroundSpellTrinket(1, Me.CurrentTarget.Location, P.myPrefs.Trinket1Use && nextTrinketTimeAllowed <= DateTime.Now)) return true;
            if (await CastGroundSpellTrinket(2, Me.CurrentTarget.Location, P.myPrefs.Trinket2Use && nextTrinketTimeAllowed <= DateTime.Now)) return true;
            if (await Cast(MANGLE, gotTarget && !spellOnCooldown(MANGLE) && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(THRASH, gotTarget && BearThrashConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(MAUL, gotTarget && BearMaulConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(LACERATE, gotTarget && BearLacerateConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(PULVERIZE, gotTarget && BearPulverizeConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            return false;
        }
        ...
        public static async Task<bool> FeralRotationCoroutine()
        {
            ...
            if (!AutoBot && Me.Mounted) return false;
            if (await RemoveRooted(FERALFORM, MeIsRooted && gotTarget && !IsInMeleeRange(Me.CurrentTarget))) return true;
            ...
            if (await Cast(MIGHTY_BASH, gotTarget && MightyBashConditions(Me.CurrentTarget) && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(WAR_STOMP, gotTarget && WarStompConditions(Me.CurrentTarget) && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await CastBuff(SAVAGE_ROAR, gotTarget && SavageRoarConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await CastBuff(TIGERS_FURY, gotTarget && TigersFuryConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await CastBuff(BERSERK, gotTarget && BerserkConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await CastBuff(INCARNATION_CAT, gotTarget && IncarnationCatConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(FORCE_OF_NATURE, gotTarget && ForceOfNatureConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(FEROCIUOS_BITE, gotTarget && FerociousBiteConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            ...
            if (await CastGroundSpellTrinket(1, Me.CurrentTarget.Location, P.myPrefs.Trinket1Use && nextTrinketTimeAllowed <= DateTime.Now)) return true;
            if (await CastGroundSpellTrinket(2, Me.CurrentTarget.Location, P.myPrefs.Trinket2Use && nextTrinketTimeAllowed <= DateTime.Now)) return true;
            if (await Cast(RIP, gotTarget && RipConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(RAKE, gotTarget && RakeConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(THRASH, gotTarget && ThrashConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(SHRED, gotTarget && ShredConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await Cast(SWIPE, gotTarget && SwipeConditions && IsInMeleeRange(Me.CurrentTarget))) return true;
            return false;
        }
