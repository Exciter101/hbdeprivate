...
namespace Styx.Bot.CustomBots
{
    public partial class LazyRaider : BotBase
    {
        ...
        private static void GroupWeighTargetsFilter(List<Targeting.TargetPriority> units)
        {
            ...
            WoWGuid guidMyCurrentTarget = Me.CurrentTargetGuid;
            WoWGuid guidTank = Tank.Guid;
            WoWGuid guidTankCurrentTarget = Tank.Player != null ? Tank.Player.CurrentTargetGuid : WoWGuid.Empty;
            ...
            foreach (var o in units)
            {
                ...
                if (LazyRaiderSettings.Instance.AutoTargetOnlyIfNotValidTarget && guidMyCurrentTarget.IsValid && u.Guid == guidMyCurrentTarget)
                {
                    o.Score = 9999;
                }
                else if (ImaTank)
        ...
        private static void PvpWeighTargetsFilter(List<Targeting.TargetPriority> units)
        {
            ...
            foreach (var p in units)
            {
                ...
                if (!u.IsAlive)
                    p.Score = 0;
                else if (LazyRaiderSettings.Instance.AutoTargetOnlyIfNotValidTarget && guidMyCurrentTarget.IsValid && u.Guid == guidMyCurrentTarget)
                    p.Score = 9999;
                else 
