	...
	public class TheVaultOfMysteries : Dungeon
	{
		...
		public Composite TheSpiritKingsEncounter()
		{
			...
			return new PrioritySelector(
				...
				// stack on tank when Qiang is active to split damge from Massive Attacks
								new Decorator(
									ctx =>
									activeBoss.CastingSpellId != annihilateId && activeBoss.CurrentTargetGuid.IsValid && activeBoss.CurrentTarget.Distance > 10 &&
		...
		public Composite ElegonEncounter()
		{
			...
			return new PrioritySelector(
				...
				new Decorator(
					ctx => !IsPlatformWalkable && _elegonRoomCenterLoc.Distance(Me.Location) < ElegonInnerCircleRadius && Me.TransportGuid.IsValid,
					new Action(
