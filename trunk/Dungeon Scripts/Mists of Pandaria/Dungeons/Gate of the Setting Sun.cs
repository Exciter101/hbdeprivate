...
namespace Bots.DungeonBuddy.Dungeon_Scripts.Mists_of_Pandaria
{
    ...
    public class GateOfTheSettingSun : Dungeon
    {
	    ...
	    public Func<WoWPoint, Task<bool>> HandleStuckAtBrokenWallSection()
	    {
		    return async loc =>
			{
				...
				var moveTo = new WoWPoint(1090.066, 2311.466, 331.6075);
        ...
        public async Task<bool> ElevatorBehavior(WoWPoint destination)
        {
            ...
            if (myFloorLevel != destinationFloorLevel)
            {
                ...
                if (!elevatorIsReadyToBoard && !elevatorInUse && myloc.DistanceSqr(elevatorWaitLoc) <= 20 * 20 && !Me.TransportGuid.IsValid && lever != null && lever.CanUse())
                {
                ...
                if (elevatorIsReadyToBoard && myloc.DistanceSqr(elevatorBoardLoc) > 2 * 2 && !Me.TransportGuid.IsValid)
                {
                    Logger.Write("[Elevator Manager] Boarding Elevator");
                    // avoid getting stuck on lever
                    Navigator.MoveTo(elevatorBoardLoc);
                }
                else if (elevatorIsReadyToBoard && myloc.DistanceSqr(elevatorBoardLoc) <= 2 * 2 && !Me.TransportGuid.IsValid)
                {
                    Logger.Write("[Elevator Manager] Jumping");
                    WoWMovement.Move(WoWMovement.MovementDirection.JumpAscend);
                    WoWMovement.MoveStop(WoWMovement.MovementDirection.JumpAscend);
