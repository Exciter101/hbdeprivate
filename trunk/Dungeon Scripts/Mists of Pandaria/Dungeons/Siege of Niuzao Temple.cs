			...
			AddAvoidObject(
				...
				o =>
				{
					...
					{ // run away from current target of boss 
						if (dashingStrikeTimer.IsFinished && unit.CurrentTargetGuid.IsValid)
						{
			...
			return async boss =>
			{
				...
				var keg =
					ObjectManager.GetObjectsOfType<WoWUnit>()
						.Where(
							u =>
								u.Entry == 61817 && u.HasAura("Mantid Barrel Active") && !u.TransportGuid.IsValid &&
		...
		private WoWPoint GetBestBombThrowLocation()
		{
			using (StyxWoW.Memory.AcquireFrame())
			{
				...
				var targets = (from unit in ObjectManager.GetObjectsOfType<WoWUnit>()
							   where unit.Entry == SikthikSoldierId && unit.IsAlive && unit.CurrentTargetGuid.IsValid
