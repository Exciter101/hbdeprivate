...
// ReSharper restore CheckNamespace
{
    ...
    public class Auchindoun : Dungeon
	{
        ...
        public override void RemoveTargetsFilter(List<WoWObject> units)
		{
			units.RemoveAll(
				ret =>
				{
				    ...
				    if (unit.Entry == MobId_VigilantKaathar && InConsecratedLightPhase && !Me.IsHealer())
				        return true;
        ...
        public override async Task<bool> HandleMovement(WoWPoint location)
        {
            ...
            var destInTerogorsRoom = location.DistanceSqr(TerogorRoomCenter) < TerogorRoomRadiusSqr;
            var meInTerogorsRoom = myLoc.DistanceSqr(TerogorRoomCenter) < TerogorRoomRadiusSqr;
            var destPlatform = PlatformAreas.FirstOrDefault(p => p.AreaCenter.DistanceSqr(location) < p.AreaRadiusSqr);
        ...
        private bool InConsecratedLightPhase;
        WoWGuid _lastHolyShieldGuid ;
        private List<Vector2[]> _sanctifiedStrikePolygons;
        ...
        public Func<WoWUnit, Task<bool>> VigilantKaatharEncounter()
        {
            ...
            WoWPoint consecratedLightMoveToLoc = WoWPoint.Zero;
            ...
            return async boss =>
            {
                ...
                var holyShield = ObjectManager.GetObjectsOfType<WoWUnit>().FirstOrDefault(g => g.Entry == MobId_HolyShield);

                if (holyShield == null)
                {
                    if (losTimer.IsRunning)
                        losTimer.Reset();
                    InConsecratedLightPhase = false;
                }
                else
                {
                    if (!losTimer.IsRunning)
                    ...
                    consecratedLightMoveToLoc = CalculateConsecratedLightLosPoint(boss, holyShield);

                    if (boss.CastingSpellId == SpellId_ConsecratedLight || boss.HasAura("Consecrated Light"))
                    {
                        InConsecratedLightPhase = true;
                    }
                    else if (losTimer.ElapsedMilliseconds < 7000)
                    {
                        var estimateTimeTillCast = 7000 - losTimer.ElapsedMilliseconds;

                        var timeToGetToLoc = consecratedLightMoveToLoc.Distance(Me.Location) / Me.MovementInfo.ForwardSpeed * 1000;
                        InConsecratedLightPhase = estimateTimeTillCast < timeToGetToLoc;
                    }
                }

                // LOS the consecrated light at the holy shield.
                if (await ScriptHelpers.StayAtLocationWhile(
                    () => InConsecratedLightPhase && consecratedLightMoveToLoc != WoWPoint.Zero && ScriptHelpers.IsBossAlive("Vigilant Kaathar"),
                    consecratedLightMoveToLoc,
                    precision: 1))
                {
                    return true;
                }
                ...
                if (!InConsecratedLightPhase && await ScriptHelpers.TankUnitAtLocation(roomCenter, 8))
                    return true;
        ...
        private WoWPoint CalculateConsecratedLightLosPoint(WoWUnit boss, WoWUnit shield)
        {
        ...
        private float PointToPolygonDistance(WoWPoint point, Vector2[] polygon)
        {
        ...
        private List<Vector2[]> GetSanctifiedStrikePolygons(WoWPoint center, float radius)
        {
        ...
        public Func<WoWUnit, Task<bool>> SoulbinderNyamiEncounter()
        {
            ...
            return async boss =>
            {
                ...
                if (soulVesel != null && await ScriptHelpers.StayAtLocationWhile(
