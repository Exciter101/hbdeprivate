...
namespace Kitty
{
    public partial class KittyMain : CombatRoutine
    {
        ...
        {
            if (Me.IsCasting || HKM.pauseRoutineOn || HKM.manualOn) return false;
            if (await RemoveRooted(BEAR_FORM, MeIsRooted && MeIsGuardian && !IsInMeleeRange(Me.CurrentTarget))) return true;
            if (await RemoveRooted(FERALFORM, MeIsFeral && MeIsRooted && !IsInMeleeRange(Me.CurrentTarget))) return true;
            ...
            if (await Cast(RAKE, gotTarget && IsInMeleeRange(Me.CurrentTarget) && MeIsFeral && P.myPrefs.PullProwlAndRake)) return true;
            if (await Cast(SHRED, gotTarget && IsInMeleeRange(Me.CurrentTarget) && MeIsFeral && P.myPrefs.PullProwlAndShred)) return true;
            if (await Cast(RAKE, gotTarget && IsInMeleeRange(Me.CurrentTarget) && MeIsFeral && !P.myPrefs.PullProwlAndRake && spellOnCooldown(FF))) return true;
            if (await Cast(SHRED, gotTarget && IsInMeleeRange(Me.CurrentTarget) && MeIsFeral && !P.myPrefs.PullProwlAndShred && spellOnCooldown(FF))) return true;
            //guardian
            ...
            if (await Cast(LACERATE, gotTarget && IsInMeleeRange(Me.CurrentTarget) && MeIsGuardian)) return true;
            //moonkin - lowbie
            if (await Cast(MOONFIRE, gotTarget && Me.CurrentTarget.Distance <= 39 && (MeIsBoomkin || MeIsLowbie) && !debuffExists(MOONFIRE, Me.CurrentTarget))) return true;
            if (await Cast(WRATH, gotTarget && Me.CurrentTarget.Distance <= 39 && (MeIsBoomkin || MeIsLowbie))) return true;
        ...
        #region spec
        public static bool MeIsFeral { get { return Me.Specialization == WoWSpec.DruidFeral; } }
        ...
        public static bool MeIsLowbie { get { return Me.Specialization == WoWSpec.None; } }
        #endregion
    }
}
