...
namespace Bots.DungeonBuddy.Dungeon_Scripts.Mists_of_Pandaria
{
	public class LastStandOfTheZandalari : Dungeon
	{
		...
		{
			return point.X >= 5475 && point.X <= 5645 && point.Y >= 6247.218 && point.Y <= 6278.957;
		}

		bool AtEastWestBridge(WoWPoint point)
		{
			return point.Y >= 6248 && point.Y <= 6041.935 && point.X <= 5448 && point.X >= 5414;
		}
		...
		public Composite JinrokhTheBreakeBehavior()
		{
			...
			return new PrioritySelector(
				...
				new Decorator(
					ctx => !Me.IsMelee() && !Me.HasAura("Fluidity") && nonElectrifiedConductiveWater != null,
					new PrioritySelector(
		...
		private readonly uint[] _tier1KillpriorityIds ={ FarrakiWastewalkerId, GurubashiVenomPriestId, DrakkariFrozenWarlordId, AmanishiBeastShamanId };

		[EncounterHandler(68476, "Horridon", BossRange = 200)]
		public Func<WoWUnit, Task<bool>> HorridonBehavior()
		{
            ...
            AddAvoidObject(
                ...
                o => o.Entry == HorridonId && o.ToUnit().IsAlive,
                o => WoWMathHelper.CalculatePointInFront(o.Location, o.Rotation, 25));
            
            // avoid Horridon's back side becasue of tail swipe			
            AddAvoidObject(
                ctx => true,
                // Increase the radius when moving to prevent stop/go behavior when standing at the edge of avoid and boss is moving
		    ...
		    return async boss =>
		    {
                // Idle if trapped on wrong side of door.
		        if (boss.Combat && Me.Y > 5865)
		            return true;
		        return false;
		    };
		}
