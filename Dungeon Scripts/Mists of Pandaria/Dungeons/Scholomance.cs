	...
	public class Scholomance : Dungeon
	{
	    ...
	    public override void IncludeLootTargetsFilter(List<WoWObject> incomingunits, HashSet<WoWObject> outgoingunits)
	    {
	        foreach (var obj in incomingunits)
	        {
                ...
                if (gObj != null)
                {
                    if (_chestIds.Contains(obj.Entry) && gObj.CanUse() && DungeonBuddySettings.Instance.LootMode != LootMode.Off)
		...
		private readonly Dictionary<WoWGuid, HarshLessonLogEntry> _partyHarshLessonsLog = new Dictionary<WoWGuid, HarshLessonLogEntry>();
        ...
        private uint[] _chestIds = new uint[] { 211277, 211278 };
		...
		[EncounterHandler(59080, "Darkmaster Gandling", BossRange = 120, Mode = CallBehaviorMode.Proximity)]
		public Func<WoWUnit, Task<bool>> DarkmasterGandlingEncounter()
		{
		    ...
		    var leftDoorEdge = new WoWPoint(59.63355, 54.7275, 87.01311);
		    var rightDoorEdge = new WoWPoint(59.48849f, 62.91311f, 87.03379);
		    var pointInsideRoom = WoWMathHelper.GetRandomPointInCircle(new WoWPoint (54.8228, 57.91502, 87.17212), 3);
		    bool runInsideRoom = false;

		    return async boss =>
		    {
		        _darkmasterGandling = boss;
		        ...
		        if (!runInsideRoom && leader != null && !leader.IsMe &&
		            ...
		            && leader.Z < 100 && !Me.Location.IsPointLeftOfLine(leftDoorEdge, rightDoorEdge) &&
		        ...
		        // get close to the boss if not 'taking a harsh lesson'
                if (Me.IsFollower() && boss.Combat
		        ...
		        if (IsTakingHarshLesson(Me) && Me.IsHealer())
		        {
		            var freshTestSubjects = (from unit in ObjectManager.GetObjectsOfType<WoWUnit>()
		            ...
		            var dispellTarget =
		                freshTestSubjects.FirstOrDefault(
		                    u =>
		                        ObjectManager.GetObjectsOfType<WoWUnit>()
		                            .Any(o => o.IsHostile && o.Location.Distance(u.Location) < 8));

		            var dispell = ScriptHelpers.GetDefensiveMagicDispel();
		            if (dispellTarget != null)
		            {
		                // dispell a fresh test subject to cause it to explode and do AOE damage to nearby hostiles.
		                if (dispell != null && SpellManager.CanCast(dispell, dispellTarget, true, false))
		                {
		                    SpellManager.Cast(dispell, dispellTarget);
		                    return true;
		                }
		                // move to dispell target if not in range or los.
		                if (dispell != null && (dispellTarget.Distance > 30 || !dispellTarget.InLineOfSpellSight)
		                    && (await CommonCoroutines.MoveTo(dispellTarget.Location)).IsSuccessful())
		                {
		                    return true;
		                }
		            }
		            else if (dispell != null && freshTestSubjects.Any() && freshTestSubjects[0].Distance > 4
		                    && (await CommonCoroutines.MoveTo(freshTestSubjects[0].Location)).IsSuccessful())
		            {
		                return true;
		            }
		        }
